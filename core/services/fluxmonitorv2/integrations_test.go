...
func checkLogWasConsumed(t *testing.T, fa fluxAggregatorUniverse, ds sqlutil.DataSource, pipelineSpecID int32, blockNumber uint64, cfg pg.QConfig) {
	t.Helper()
...
func TestFluxMonitor_Deviation(t *testing.T) {
	...
	"""
		`
			...
			checkLogWasConsumed(t, fa, app.GetDB(), jobID, receiptBlock, app.GetConfig().Database())
